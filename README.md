...
- **vCenter 管理** — CRUD、连接测试、pyVmomi 扫描采集虚拟机清单（15 个字段：数据中心/集群/ESXi/资源池/文件夹/名称/电源/IP/MAC/网络/VLAN/OS/CPU/内存）
...
│   │   ├── models/              # User, Switch, ScanResult, RouteTable, ScanLog, Subnet, History, VCenter, VMInventory
│   │   ├── schemas/             # Pydantic 请求/响应
│   │   ├── api/                 # auth, switches, results, history, scan_logs, dashboard, subnets, search, users, vcenters
│   │   ├── services/            # scanner_service, subnet_service, history_service, scheduler_service, vcenter_scanner_service
│   │   └── utils/               # security (JWT, bcrypt)
│   ├── seed.py                  # 默认用户初始化
│   └── requirements.txt
├── frontend/
│   ├── src/
│   │   ├── views/               # Login, Dashboard, SwitchList, SwitchDetail, VCenterList, VCenterDetail, Results, Routes, SubnetManage, SearchResult, History, UserManage, Profile
│   │   ├── components/          # AppLayout, SwitchFormDialog, VCenterFormDialog
│   │   ├── api/                 # Axios 封装（auth, switches, results, history, users, vcenters）
│   │   ├── store/               # Pinia auth
